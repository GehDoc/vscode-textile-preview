...
import { getEndLineNumber, getLineNumber, TextileEngine } from '../textileEngine';
...
export default class TextileSmartSelect implements vscode.SelectionRangeProvider {
...
function getBlockTokensForPosition(tokens: Token[], position: vscode.Position, parent?: vscode.SelectionRange): Token[] {
	const enclosingTokens = tokens.filter((token) => {
		console.table(token);
		const startLine = getLineNumber(token)
		const endLine = getEndLineNumber(token)
		return startLine !== undefined && endLine !== undefined &&
		(startLine <= position.line && endLine > position.line) && (!parent || (startLine >= parent.range.start.line && endLine <= parent.range.end.line + 1)) && isBlockElement(token)});
	if (enclosingTokens.length === 0) {
		return [];
	}
	const sortedTokens = enclosingTokens.sort((token1, token2) => (getEndLineNumber(token2)! - getLineNumber(token2)!) - (getEndLineNumber(token1)! - getLineNumber(token1)!));
	return sortedTokens;
}
...
function isList(token: Token): boolean {
	return token[0] ? ['ol', 'li', 'ul'].includes(token.type) : false;
}

function isBlockElement(_token: Token): boolean {
	return true //!['li', 'pre', 'p', 'div', 'blockquota', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'].includes(token[0]);
}
