...
import { getVisibleLine, TextileFileTopmostLineMonitor } from '../util/topmostLineMonitor';
...
export class TextilePreview extends Disposable {
	...
	public static async revive(
		...
		preview.editor.webview.options = TextilePreview.getWebviewOptions(resource, contributionProvider.contributions);
	...
	private constructor(
		...
		this._register(this.editor.webview.onDidReceiveMessage((e: CacheImageSizesMessage | RevealLineMessage | DidClickMessage | ClickLinkMessage | ShowPreviewSecuritySelectorMessage | PreviewStyleLoadErrorMessage) => {
			...
			switch (e.type) {
				...
				case 'previewStyleLoadError':
					vscode.window.showWarningMessage(localize('onPreviewStyleLoadError', "Could not load 'textile.styles': {0}", e.body.unloadedStyles.join(', ')));
					break;
			}
		}));
		...
		this._register(vscode.window.onDidChangeActiveTextEditor(editor => {
			if (editor && isTextileFile(editor.document) && !this._locked) {
	...
	public matches(otherPreview: TextilePreview): boolean {
	...
	public toggleLock() {
		...
		this.editor.title = TextilePreview.getPreviewTitle(this._resource, this._locked);
	}
	...
	private updateForView(resource: vscode.Uri, topLine: number | undefined) {
		...
		if (typeof topLine === 'number') {
			this._logger.log('updateForView', { textileFile: resource });
			this.line = topLine;
			this.postMessage({
				type: 'updateView',
				line: topLine,
	...
	private async doUpdate(): Promise<void> {
		...
		const textileResource = this._resource;
		...
		try {
			document = await vscode.workspace.openTextDocument(textileResource);
		} catch {
		...
		const pendingVersion = new PreviewDocumentVersion(textileResource, document.version);
		if (!this.forceUpdate && this.currentVersion && this.currentVersion.equals(pendingVersion)) {
			if (this.line) {
				this.updateForView(textileResource, this.line);
			}
			return;
		}
		...
		if (this._resource === textileResource) {
			const self = this;
			const resourceProvider: WebviewResourceProvider = {
				asWebviewUri: (resource) => {
					return this.editor.webview.asWebviewUri(normalizeResource(textileResource, resource));
				},
	...
	private static getWebviewOptions(
		resource: vscode.Uri,
		contributions: TextileContributions
	): vscode.WebviewOptions {
		return {
			enableScripts: true,
			localResourceRoots: TextilePreview.getLocalResourceRoots(resource, contributions)
		};
	}
	...
	private setContent(html: string): void {
		...
		this.editor.webview.options = TextilePreview.getWebviewOptions(this._resource, this._contributionProvider.contributions);
	...
	private async onDidClickPreviewLink(path: string, fragment: string | undefined) {
		...
		if (openLinks === 'inPreview') {
			const textileLink = await resolveLinkToTextileFile(path);
			if (textileLink) {
				if (fragment) {
					this.scrollToFragment = fragment;
				}
				this.update(textileLink);
				return;
			}
		}

		vscode.commands.executeCommand('_textile.openDocumentLink', { path, fragment });
	}
