 ...
 */
export class TextileDocumentInfoCache<T> extends Disposable {
	...
	public constructor(
		...
		this._register(this.workspace.onDidChangeTextileDocument(doc => this.invalidate(doc)));
	...
	private loadDocument(resource: vscode.Uri): Promise<ITextDocument | undefined> {
		...
		const p = this.workspace.getOrLoadTextileDocument(resource);
...
/**
 * Cache of information across all textile files in the workspace.
 *
 * Unlike {@link TextileDocumentInfoCache}, the entries here are computed eagerly for every file in the workspace.
 * However the computation of the values is still lazy.
 */
export class TextileWorkspaceInfoCache<T> extends Disposable {
	...
	private async ensureInit(): Promise<void> {
		if (!this._init) {
			...
			this._register(this.workspace.onDidCreateTextileDocument(this.onDidChangeDocument, this));
			this._register(this.workspace.onDidDeleteTextileDocument(this.onDidDeleteDocument, this));
		}
		await this._init;
	}
	...
	private async populateCache(): Promise<void> {
		const textileDocumentUris = await this.workspace.getAllTextileDocuments();
