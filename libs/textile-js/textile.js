...
var mergeConcatClassname = __webpack_require__(12);
...
function parsePhrase(src, options, charPosToLine, charOffset) {
  ...
  do {
    ...
    var srcSlot = src.getPos(); // lookbehind => /([\s>.,"'?!;:])$/
    ...
    if ((m = rePhrase.exec(src)) && (boundary || m[1])) {
      ...
      var rx = re.compile("".concat(mMid, "(").concat(re.escape(tok), ")").concat(mEnd)); // no line number here: they are not block level elements

      if ((m = rx.exec(src)) && m[1]) {
        var srcPos = src.getPos();
        src.advance(m[0]);
    ...
    if ((m = reImage.exec(src)) || (m = reImageFenced.exec(src))) {
      ...
      var img = ['img', mergeConcatClassname(attr, options.showOriginalLineNumber ? addLineNumber({}, options, charPosToLine, charOffset, srcSlot) : undefined)];
      attr.src = m[2];
    ...
    if (m = testOpenTag(src)) {
      ...
      var _srcPos = src.getPos();

      var tag = m[1];
      var single = m[3] || m[1] in singletons;
      var element = [tag];

      var _attr = m[2] ? parseHtmlAttr(m[2]) : undefined;

      if (single) {
        // single tag
        var pbaLineNumber = options.showOriginalLineNumber ? addLineNumber({}, options, charPosToLine, charOffset, srcSlot) : undefined;

        if (_attr) {
          element.push(mergeConcatClassname(_attr, pbaLineNumber));
        ...
        if (m = reEndTag.exec(src)) {
          var _pbaLineNumber = options.showOriginalLineNumber ? addLineNumber({}, options, charPosToLine, charOffset, srcSlot, srcSlot + (m[0].length > 0 ? m[0].length - 1 : 0)) : undefined;

          if (_attr) {
            element.push(mergeConcatClassname(_attr, _pbaLineNumber));
          } else if (_pbaLineNumber) {
            element.push(_pbaLineNumber);
          }
          ...
          if (tag === 'code') {
            ...
            element = element.concat(parsePhrase(m[1], options, charPosToLine, charOffset + _srcPos));
          }
...
function paragraph(s, tag, pba, linebreak, options, charPosToLine, charOffset) {
  tag = tag || 'p';
  var out = []; // FIXME : this may need some work for correct offset computation
  ...
  s.split(/(?:\r?\n){2,}/).forEach(function (bit, i) {
    if (tag === 'p' && /^\s/.test(bit)) {
      ...
      out.push(pba ? [tag, pba].concat(parsePhrase(bit, options, charPosToLine, charOffset)) : [tag].concat(parsePhrase(bit, options, charPosToLine, charOffset)));
    }
  });
  return out;
}
    ...
    if (m = reBlock.exec(src)) {
      ...
      if (m = /^\.(\.?)(?:\s|(?=:))/.exec(src)) {
        ...
        if (blockType === 'bq') {
          ...
          list.add(['p', pba, ['a', {
            'href': '#fnr' + fnid
          }, ['sup', fnid]], ' '].concat(parsePhrase(m[1], options, charPosToLine, src.getSlot())));
        } else {
          // heading | paragraph
        ...
        if (m[3] || tag in singletons) {
          ...
          if (_x.tag === tag) {
            ...
            if (/^\s*(\n|$)/.test(src)) {
              ...
              if (options.showOriginalLineNumber) {
                _elm2.push(addLineNumber(m[2] ? parseHtmlAttr(m[2]) : {}, options, charPosToLine, 0, _srcSlot, src.getPos() - 1));
              } else {
                if (m[2]) {
              ...
              if (tag === 'script' || tag === 'style') {
                ...
                var localCharOffset = _t2.length > 1 ? _t2[_s].pos : 0;

                if (options.showOriginalLineNumber) {
                  var removedSrc = _inner.match(/^\n+/);
                ...
                var innerElm = isBlock ? parseFlow(innerHTML, options, charPosToLine ? charPosToLine[_srcSlot + localCharOffset] : undefined) : parsePhrase(innerHTML, extend({}, options, {
                  breaks: false
                }), charPosToLine, _srcSlot + localCharOffset);
...
function parseDefList(src, options, charPosToLine, charOffset) {
  ...
  while (m = reItem.exec(src)) {
    ...
    while (terms.length) {
      ...
      deflist.push('\t', ['dt'].concat(addLineNumber({}, options, charPosToLine, charOffset, src.getPos() + localCharOffset), parsePhrase(term.trim(), options, charPosToLine, charOffset)), '\n');
    ...
    deflist.push('\t', ['dd'].concat(addLineNumber({}, options, charPosToLine, charOffset, src.getPos() + localCharOffset), /=:$/.test(def) ? parseFlow(def.slice(0, -2).trim(), options, options.showOriginalLineNumber ? charPosToLine[(charOffset || 0) + localCharOffset + src.getPos()] : 0) : parsePhrase(def, options, charPosToLine, (charOffset || 0) + localCharOffset + src.getPos())), '\n');
    src.advance(m[0]);
  }
...
var reHead = /^table(_?)([^\n]*?)\.(?:[ \t](.+?))?\s*\n/;
var reRowCapture = re.compile(/^(\|([~^-][:txattr:])\.\s*\n)?([:txattr:]\.[^\n\S]*)?(\|)(.*?)\|[^\n\S]*(\n|$)/, 's');
var reCaption = /^\|=([^\n+]*)\n/;
var reColgroup = /^\|:([^\n+]*)\|[\r\t ]*\n/;
var reRowgroup = /^\|([\^\-~])([^\n+]*)\.[ \t\r]*\n/;
var charToTag = {
    ...
    if (m = reColgroup.exec(src)) {
      ...
      extended++;
        ...
        extended++;
          ...
          var localCharOffset = 0 + (m[1] ? m[1].length : 0) + (m[2] ? m[2].length : 0) + (m[3] ? m[3].length : 0) + (m[4] ? m[4].length : 0);
